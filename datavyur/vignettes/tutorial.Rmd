...
Sometimes typos are invalid codes make it into the final export. You can check which codes are invalid by using the function `check_codes`. First let us load the `childhands` column data.
...
Then we can make a list of column names and the allowed set of codes for each column in the data `child_hands`.
Notice how that names of each item in the list below is the exact column name from the data.frame `child_hands`.
And for each item it contains a vector of valid codes only. We will use this list to pass to the function `check_codes`.
...
The result of using the `check_codes` function is a list with two items, the new data with bad codes set to `NA`, and list called `bad_codes` which for each item in `child_hands_codes`, will return the indices and codes that were found to be invalid.
...
The function returns a list of new data with bad timestamps as `NA`, a list of timestamps out of range (if specified in the arguments), and a list of timestamps with bad durations.
...
You can also align all the columns in your data folder by time, or just a select few of the columns. It is best if you know the framerate of the videos, that's because to align by time, the `temporal_align` function converts timestamps to frame number, then matches frames across columns and codes. Since timestamps are now frames, you'll receive a very large dataset for each file and each frame number, unless `keep.frames` is set to `FALSE`.
...
If you want to align data across columns by the cell number instead of by time, you can use the function `ordinal_align`. This may be useful if the same cells are copied across multiple columns, but the timestamps don't change, but the codes in each column are different and you want to combine them.
