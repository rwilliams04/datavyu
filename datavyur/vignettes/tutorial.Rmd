...
This section will outline how to use some of the basic functions found in the `datavyur` package.
To follow along, make sure you know where your `.csv` files have been saved when you used the `datavyu2csv.rb` script to convert `.opf` columns to individual `.csv` files. If you don't have `.csv` files to work with, you can use the default ones built into this package, as shown in the examples.
...
list.files(getOption("datavyur.folder"))
```

An alternative to setting the `datavyur.folder` R option is to just create an R object that holds the path to your data folder, like so:
...
## Viewing `.opf` column information
...
You can save any R `data.frame` to a spreadsheet that can be opened in Excel. You don't need the `datavyur` packages for this, and comes with standard R. You can do this for any R `data.frame`. This will write the data `z2`
...
If you have columns that are originally nested within each other, you can get them back to this format by using the function `merge_nested`. The example below will attempt to merge two Datavyu columns by timestamps (even though these example data aren't really nested). Merging nested columns only works if you have two separate columns in your original `.opf` file, with cells in one column being nested within cells in another column. At the moment, it won't work with 3 or more nested columns (e.g., level 3 nested within level 2, which is nested within level 1).

All you need to really supply for the `merge_nested` function is the column name of the top level and the column name of the lower level.

```{r}
z1 <- merge_nested(child_hands, parent_hands)
```

Since these exaple data aren't really nested, you'll get a lot of rows that won't merge. But you can still include them if you want. This is the default behavior of the function.
